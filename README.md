...
- **AI Agent Integration (MCP)** - exposes `list_signals`, `get_receivers`, `find_signal_by_sender`, and `inspect_receiver` tools so AI agents (Cursor, Claude, etc.) can introspect your signal landscape via [dj-control-room](https://github.com/django-control-room/dj-control-room)'s MCP server
...
Ships `list_signals`, `get_receivers`, `find_signal_by_sender`, and `inspect_receiver` tools that [dj-control-room](https://github.com/django-control-room/dj-control-room)'s MCP server exposes to AI agents (Cursor, Claude, etc.), so they can look up signals, receivers, and source locations without grepping your codebase.
